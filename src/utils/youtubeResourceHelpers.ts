...
function thumbnailURL(
  thumbnailDetails: gapi.client.youtube.ThumbnailDetails | null | undefined,
): string {
  if (!thumbnailDetails) {
...
function makeVideoURL(
  ...
  return `https://www.youtube.com/watch?v=${videoId}${
    playlistId === undefined ? "" : `&list=${playlistId}`
  }${playlistPosition === undefined ? "" : `&position=${playlistPosition}`}`;
}
...
export {
  ...
  makeVideoURL,
  makeChannelURL,
  UNAVAILABLE_THUMBNAIL_URL,
  VIDEO_THUMBNAIL_DIMENSIONS,
};
