...
function hasProperty<KnownT extends object, WithPropT extends PropertyKey>(
  obj: KnownT,
  prop: WithPropT,
): obj is KnownT & Record<WithPropT, unknown> {
...
function isUnauthenticatedError(error: any): boolean {
  ...
  return (
    (resultError.code === 401 &&
      resultError.errors?.[0]?.message === "Invalid Credentials") ||
