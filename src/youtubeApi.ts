...
function isVideoAvailableInCountry(video: Video, countryCode: string): boolean {
  ...
  if (regionRestriction.allowed) {
    return (
      ...
      regionRestriction.allowed.includes(countryCode)
    );
  } else {
    ...
    return (
      ...
      !regionRestriction.blocked.includes(countryCode)
    );
  }
}

async function fetchVideosInPlaylist(
  playlistId: string,
...
async function fetchUnavailablePublicPlaylistItems(
  playlistId: string,
 ...
 * @returns
 */
async function fetchUnavailablePlaylistItems(
  playlistId: string,
  userChannelId: string | null,
  userCountryCode: string,
): Promise<PlaylistItem[]> {
  const playlistItemResponses = await fetchVideosInPlaylist(playlistId);
  return filterAvailablePlaylistItems(
    playlistItemResponses,
    userChannelId,
    userCountryCode,
  );
}
 ...
 * @returns
 */
async function filterAvailablePlaylistItems(
  playlistItemResponses: PlaylistItemListResponse[],
  userChannelId: string | null,
  userCountryCode: string,
): Promise<PlaylistItem[]> {
  ...
  for (const response of playlistItemResponses) {
    ...
    for (const item of response.items) {
      ...
      if (
        ...
        item.snippet?.videoOwnerChannelId !== userChannelId
      ) {
  ...
  while (startIndex < uniqueVideoIds.length) {
    batch.add(
      gapi.client.youtube.videos.list({
        ...
        id: uniqueVideoIds
          .slice(startIndex, startIndex + MAX_VIDEO_IDS_PER_REQUEST)
          .join(","),
        maxResults: 50,
      }),
    );
  ...
  for (const responseId in batchResults) {
    const response = batchResults[
      responseId
    ] as gapi.client.Response<gapi.client.youtube.VideoListResponse>;
...
export {
  fetchOwnedPlaylists,
  filterAvailablePlaylistItems as includeUnavailablePlaylistItems,
