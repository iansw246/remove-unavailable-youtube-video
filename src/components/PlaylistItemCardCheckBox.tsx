...
import {
  ...
  makeChannelURL,
  makeVideoURL,
} from "../utils/youtubeResourceHelpers";
...
export default function PlaylistItemCardCheckBox({
  playlistItem,
  checked,
  onChange,
}: Props) {
  const thumbnailAndDetails = useMemo(
    () => (
      <>
        <YouTubeThumbnail
          thumbnailURL={thumbnailURL(playlistItem.snippet?.thumbnails)}
          alt={playlistItem.snippet?.title + " thumbnail"}
          sx={{ marginRight: 1 }}
        />
        <Box minWidth="200px">
          <Typography>
          ...
          <Typography>
            Published on{" "}
            {new Date(playlistItem.snippet?.publishedAt || "").toLocaleString()}
          </Typography>
        </Box>
      </>
    ),
