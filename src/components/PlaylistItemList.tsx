...
export default function PlaylistItemList({
  ...
  const [checkAllCheckboxState, setCheckAllCheckboxState] =
    useState<CheckAllCheckboxState>(CheckAllCheckboxState.UNCHECKED);
  ...
  const filteredItems = useMemo(() => {
    return items.filter(
      (item): item is Required<PlaylistItem> =>
        item !== undefined &&
      ...
      {showCheckboxes ? (
        <>
          <Box
            ...
            alignItems="flex-end"
          >
            <Typography>Remove video?</Typography>
            <FormControlLabel
              control={
                <Checkbox
                  onChange={handleCheckAllPlaylistItemsButtonClick}
                  checked={
                    checkAllCheckboxState === CheckAllCheckboxState.CHECKED
                  }
                  indeterminate={
                    checkAllCheckboxState ===
                    CheckAllCheckboxState.INDETERMINATE
                  }
                />
              }
              label="Select All"
              ...
              <PlaylistItemCardCheckbox
                ...
                onChange={(e) => {
                  ...
                  if (onSelectionChanged) {
                    onSelectionChanged(
                      items.filter((_, j) => newCheckedItems[j]),
                    );
                  }
                }}
              />
