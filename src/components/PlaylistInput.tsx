...
function parseYoutubePlaylistInput(
  input: string,
  stringToUrl: (str: string) => URL | null = defaultStringToUrl,
) {
  ...
  if (
    PLAYLIST_ID_DISALLOWED_CHARACTERS.some((disallowedChar) =>
      input.includes(disallowedChar),
    )
  ) {
    return null;
  }
...
export default function PlaylistInput({ onChange }: Props) {
  const [isPlaylistInputValid, setIsPlaylistInputValid] =
    useState<boolean>(true);
  const playlistInputId = useId();

  const onPlaylistInputChange = useCallback(
  ...
  return (
    <Box>
      <TextField
        ...
        helperText={
          isPlaylistInputValid ? undefined : "Invalid playlist ID/link"
        }
      />
    </Box>
  );
