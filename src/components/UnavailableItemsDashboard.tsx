...
function createErrorAlertBody(error: unknown): React.ReactNode {
  try {
    if (isUnauthenticated(error)) {
      ...
      if (
        errorAny.type === ErrorType.General &&
        ...
        errorAny.type === ErrorType.YouTubeApiError &&
...
const UnavailableItemsDashboard = forwardRef(
  (
    {
      unavailableItems,
      playlist,
      showRemoveVideosButton: allowRemoval = true,
      onVideosRemoved,
    }: Props,
    ...
    const handleSelectedVideosChanged = useCallback(
    ...
    return (
      <div ref={ref}>
        <Typography variant="h4">
          Unavailable videos in selected playlist
        </Typography>
        <Tabs
          value={tabIndex}
        ...
        <TabPanel value={tabIndex} index={0}>
          ...
          {allowRemoval ? (
            <>
              <Button
                variant="contained"
                onClick={handleRemoveSelectedVideosButtonClick}
                sx={{ margin: 1 }}
              >
        ...
        <TabPanel value={tabIndex} index={1}>
          ...
          <ExportPlaylistItems
            ...
            playlistName={playlist.snippet?.title ?? "untitled_playlist"}
          />
        </TabPanel>
      </div>
    );
