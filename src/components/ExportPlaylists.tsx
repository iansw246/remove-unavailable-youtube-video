...
import usePlaylistItemsExportText, {
  DataFormat,
  dataFormatToFileExtension,
  isDataFormat,
} from "./usePlaylistItemsExportText";
...
export default function ExportPlaylistItems({
  playlistName,
  playlistItems,
}: Props) {
  const [playlistDataTextObjectURL, setPlaylistDataTextObjectURL] =
    useState<string>();
  const [playlistDataFormat, setPlaylistDataFormat] = useState<DataFormat>(
    DataFormat.PlainText,
  );
  const playlistItemsDataText = usePlaylistItemsExportText(
    playlistItems,
    playlistDataFormat,
  );
  ...
  useEffect(() => {
    setPlaylistDataTextObjectURL(
      URL.createObjectURL(
        new Blob([playlistItemsDataText], { type: "application/json" }),
      ),
    );
  }, [playlistItemsDataText]);
    ...
    <>
      ...
      <div>
        ...
        <Button
          variant="contained"
          component="a"
          download={`${playlistName.replaceAll(
            invalidFilenameCharactersRegex,
            "_",
          )}-unavailable videos.${
            dataFormatToFileExtension(playlistDataFormat) || ".txt"
          }`}
